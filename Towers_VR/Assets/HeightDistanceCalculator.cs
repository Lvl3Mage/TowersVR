...
public class HeightDistanceCalculator : MultipleNumberContainer
{
    ...
    { // the input id is discarded for now. It can be used to not perform the calculations of the distance if it wasn't changed
    	if(NumberList[1].value != 0){
	    	...
	    	float distancex = Mathf.Sin(xangle*Mathf.Deg2Rad)*distanceH; // calculating the horizontal distance to the point using the formula sin(alfa) = o/h
	    	float relheight = Mathf.Cos(xangle*Mathf.Deg2Rad)*distanceH;// calculating the relative height to the point using the formula cos(alfa) = a/h
